            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i>Customers Table
                    </div>
                    <div class="card-body">
                        <div class="btn-toolbar justify-content-between form-group" role="toolbar">
                            <router-link :to="{name: 'CustomerCreate'}" class="btn btn-success">Create new customer</router-link>
                        ...
                        <table class="table table-striped table-bordered table-responsive-sm table-sm" v-bind:class="{'table-loading' : loading}">
                            ...
                            <tr>
                                ...
                                <th class="sorting" v-on:click="sortCol('name')" v-bind:class="[filter.sort == 'name' ? filter.dir : '']">Name</th>
                                <th class="sorting" v-on:click="sortCol('cnp')" v-bind:class="[filter.sort == 'cnp' ? filter.dir : '']">CNP</th>
                            ...
                            <tbody>
                                <tr v-for="rowData, index in listData">
                                    <td><span v-if="rowData.deleted_at != null" class="badge badge-secondary">Deleted</span> #{{rowData.id}}</td>
                                    <td>{{rowData.name}}</td>
                                    <td>{{rowData.cnp}}</td>
                                    <td>{{rowData.created_at}}</td>
                                    <td class="text-center" v-if="rowData.deleted_at == null">
                                        <router-link :to="{name: 'CustomerEdit', params: {id: rowData.id}}" class="btn btn-outline-warning btn-sm">
                                            Edit
                                        </router-link>
                                        <a href="#"
